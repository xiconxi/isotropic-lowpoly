...
int main(int argc, char** argv) {
    if(argc != 5) {
        std::cerr << "Usage: ./lowpoly_cvt ../data/gradient/1.bmp ../data/1.jpeg 5000 ../imgs/1 " << std::endl;
    }
    assert(argc == 5);
    Eigen::MatrixXd gradient;
    assert( read_density(argv[1], gradient) );
    size_t n_sample = std::stoi(argv[3]);
...
//        export_svg(Pts, std::to_string(i)+"lloyd.svg");
    ...
    export_colored_svg(std::string(argv[4])+"lowpoly_tri.svg", V2, F2, R, G, B);
    export_svg(Pts, std::string(argv[4])+"lowpoly_point.svg");
